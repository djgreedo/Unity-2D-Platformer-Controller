...
public class ColliderMotor2d : MonoBehaviour
{
    #region Public
    ...
    protected float _ignoreMovementFrames;
    ...
    protected WallState _wallInfo = new WallState();
    ...
    protected MovingPlatformState _movingPlatformState = new MovingPlatformState();
    ...
    protected bool HasFlag(CollidedSurface cs)
    {
        return (collidingAgainst & cs) != CollidedSurface.None;
    }

    virtual protected bool IsInAir()
    {
    ...
    protected bool PressingIntoLeftWall()
    {
        if (_movingPlatformState.isOnPlatform &&
            _movingPlatformState.stuckToWall == CollidedSurface.LeftWall &&
    ...
    protected bool PressingIntoRightWall()
    {
        if (_movingPlatformState.isOnPlatform &&
            _movingPlatformState.stuckToWall == CollidedSurface.RightWall &&
            normalizedXMovement > wallInteractionThreshold)
    ...
    private void HandlePostWallInteraction()
    {
        ...
        if (HasFlag(CollidedSurface.Ground))
        {
            if (motorState == MotorState.WallSliding || motorState == MotorState.FreedomState)
            {
                _velocity.y = 0;
            }
    ...
    /// </summary>
    virtual protected void HandlePreWallInteraction()
    {
        // Corner grab?
    ...
    private void ApplyMovement()
    {
        ...
        // ladder check
        if (IsOnLadder() && motorState == MotorState.FreedomState)
        {
    ...
    protected int GetFrameCount(float time)
    {
